...
export async function GET() {
  try {
    ...
    console.error('weights GET error:', e)
    return Response.json({ kpiWeight: 40, attendanceWeight: 20, sharingWeight: 15, attitudeWeight: 15, skillWeight: 10 })
  }
}
...
export async function POST(req) {
  try {
    ...
    console.error('weights POST error:', e)
    return Response.json({ error: `Gagal menyimpan: ${e.message}` }, { status: 500 })
  }
}
