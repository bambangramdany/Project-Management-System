...
export async function GET(req, { params }) {
  ...
  return NextResponse.json({
    ...
    quotationFileName: project.quotationFileName,
  ...
  if (canEdit && !isLocked) {
    ...
    await Promise.all(items.map(async (item, idx) => {
      const data = {
        ...
        qty: item.qty === '' || item.qty === null || item.qty === undefined ? null : (parseFloat(item.qty) || 0),
        unitPrice: item.unitPrice === '' || item.unitPrice === null || item.unitPrice === undefined ? null : (parseFloat(item.unitPrice) || 0),
        needsUpfrontFunding: !!item.needsUpfrontFunding,
    ...
    if (body.includesPpn !== undefined && canEditProjectValue(session.user, project)) {
      const newIncludesPpn = !!body.includesPpn
      if (newIncludesPpn !== project.includesPpn) {
        await prisma.project.update({
          where: { id: params.id },
          data: { includesPpn: newIncludesPpn },
        })
      }
    }
